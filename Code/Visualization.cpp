...
{
    std::cout << "£adowanie obrazka z pliku " + filename + "\n";

// za³aduj dane z pliku
    SDL_Surface* surface = SDL_LoadBMP(filename.c_str());
    if (!surface) 
    {
        std::cerr << "£adowanie " + filename + " FAILED: " + SDL_GetError() + "\n";
        exit(1);
    }

//sprawdzanie poprawnoœci wymiarów
    const int width = surface->w;
    const int height = surface->h;
    if (((width & (width - 1)) != 0) || ((height & (height - 1)) != 0)) 
    {
        std::cerr << "Obrazek " + filename + " ma nieprawid³owe wymiary: " << width << "x"
                << height << "\n";
        exit(1);
    }
